...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    const auto environmentBox = std::make_shared<library::Material>(L"environmentMaterial");
    environmentBox->pDiffuse = std::make_shared<library::Texture>("Content/plane.jpg");

    if (FAILED(mainScene->AddMaterial(environmentBox)))
    {
        return 0;
    }
    ...
    floor->AddMaterial(environmentBox);

    if (FAILED(mainScene->AddRenderable(L"environmentBox", floor)))
    {
        return 0;
    }
    if (FAILED(mainScene->SetVertexShaderOfRenderable(L"environmentBox", L"EnvironmentMapShader")))
    {
        return 0;
    }
    if (FAILED(mainScene->SetPixelShaderOfRenderable(L"environmentBox", L"EnvironmentMapShader")))
    {
        return 0;
    }
    ...
    XMFLOAT4 color;
    XMStoreFloat4(&color, Colors::Blue);
