...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    std::shared_ptr<YourCube2> yourCube2 = std::make_shared<YourCube2>();

    if (FAILED(game->GetRenderer()->AddRenderable(L"YourCube2", std::make_shared<YourCube2>())))
    {
        return 0;
    }
    ...
    std::shared_ptr<AdditionalCube> additionalCube = std::make_shared<AdditionalCube>();

    if (FAILED(game->GetRenderer()->AddRenderable(L"AdditionalCube", std::make_shared<AdditionalCube>())))
    {
        return 0;
    }

    if (FAILED(game->GetRenderer()->SetVertexShaderOfRenderable(L"AdditionalCube", L"MainShader")))
    {
        return 0;
    }

    if (FAILED(game->GetRenderer()->SetPixelShaderOfRenderable(L"AdditionalCube", L"MainShader")))
    {
        return 0;
    }
