	...
	*/
	...
	HRESULT Renderable::initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext)
	{
		...
		*/
		...
		// Create the m_normalBuffer  buffer***********************************
		...
		hr = pDevice->CreateBuffer(&nBufferDesc, &nData, m_normalBuffer.GetAddressOf());
		if (FAILED(hr))
			return hr;
...
/*M+M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M+++M
	...
	const std::shared_ptr<Material>& Renderable::GetMaterial(UINT uIndex) const
	{
		...
		//The type of m_aMaterials is changed, so change the function and references properly
